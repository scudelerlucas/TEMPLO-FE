...
> Instruções para CLIs que leem `AGENTS.md` (Codex CLI). O bloco abaixo é **espelho do hub**
> `scudelerlucas/Lucas-Contexto-Geral`, gravado por `scripts/sync-rules.mjs` — **não editar aqui**:
> a fonte é o `AGENTS.md` do hub, e a Action `rules-drift` acusa divergência. Conteúdo próprio
> deste repositório pode ser escrito FORA dos marcadores, que o sync não toca.
